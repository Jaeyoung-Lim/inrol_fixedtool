...
int main(int argc, char **argv)
{
	 ...
	 ros::Publisher pub_tool_wd = n.advertise<geometry_msgs::Vector3>("fixedtool/w_d", loopRate);
	 ros::Publisher pub_tool_u_c = n.advertise<geometry_msgs::Vector3>("fixedtool/u_c", loopRate);
	 ...
	 geometry_msgs::Vector3 pub_w_d_dot;
	 geometry_msgs::Vector3 pub_u_c;
	 std_msgs::Float64 cmd_thr;
    ...
    ros::Time last_request = ros::Time::now();
	 ...
	 while (ros::ok()){
	  if( current_state.mode != "OFFBOARD" &&
		  ...
		  Vector3 nu_l = delta_o*w_t; //angular velocity transformation - eq. (8)
		  Vector3 u_hat = R_t.Trans()*(u_c/m*(-1.0) + e3*9.81); //RHS of (9)
		  //Vector3 w_d;
		  Vector3 w_d_dot;
		 ...
		 // w_d = w + w_d_dot*stp;
		  ...
		  //tau = Sw_t*J*w_t + J*(w_d_dot +(w_t+w_d*(-1.0))*(-1.0)*k_t); // - eq/ (7)
		  ...
		  */
		  ...
		  pub_y_dot.twist.linear.x = y_dot.x;
		  pub_y_dot.twist.linear.y = y_dot.y*(-1.0);
		  pub_y_dot.twist.linear.z = y_dot.z*(-1.0);
		  pub_y_dot.twist.angular.x = w_t.x;
		  pub_y_dot.twist.angular.y = w_t.y;
		  pub_y_dot.twist.angular.z = w_t.z;

		  pub_w_d_dot.x = w_d_dot.x;
		  pub_w_d_dot.y = w_d_dot.y;
		  pub_w_d_dot.z = w_d_dot.z;
		  ...
		  pub_tool_wd.publish(pub_w_d_dot);
		  pub_tool_u_c.publish(pub_u_c);
		  ros::spinOnce();
		  loop_rate.sleep();
		  ++count;
	 }
