...
int main(int argc, char **argv)
{
	...
	ros::Publisher pub_act = n.advertise<mavros_msgs::ActuatorControl>("/fixedtool/actuator_control", 200);
	...
	n.param("/fixedtool/gains/att_k_star", k_star, 1.0);
	...
	while (ros::ok()){
		...
		if(estimation_mode == true){
			...
			if(ctrl_mode == true){
				...
				//nu_d_dot.z = 0;
				nu_d_dot.z = k_star*(nu_d_dot.x+nu_d_dot.x*nu.y*nu.y+2*nu.x*nu.y*nu_d_dot.y-atan(nu.x*nu_d_dot.y-nu.y/(1+nu.x*nu.x)));
			...
			if(ctrl_mode == true){
				...
				nu_d_dot.z = k_star*(nu_d_dot.x+nu_d_dot.x*nu_t.y*nu_t.y+2*nu_t.x*nu.y*nu_d_dot.y-atan(nu_t.x*nu_d_dot.y-nu_t.y/(1+nu_t.x*nu_t.x)));
