    ...
    ros::Time last_request = ros::Time::now();
	 ...
	 while (ros::ok()){
	  if( current_state.mode != "OFFBOARD" &&
		  ...
		  nu_d_dot.z = k_star*(nu_d_dot.x+nu_d_dot.x*nu_l.y*nu_l.y+2*nu_l.x*nu_l.y*nu_d_dot.y-atan(nu_l.x)*nu_d_dot.y-nu_l.y/(1+nu_l.x*nu_l.x));

		  nu_d = nu_l+nu_d_dot*stp; // integrate nu_d		  
		  nu_d.z = k_star*nu_l.x*(1+nu_l.y*nu_l.y)-atan(nu_l.x)*k_star*nu_l.y+atan(nu_l.x)*u_bar; //(16)
		  ...
		  //tau = Sw*J*w + J*w_d_dot + (w-w_d)*(-1.0)*k_t; //Calculation of moment input
