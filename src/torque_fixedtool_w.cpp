    ...
    ros::Time last_request = ros::Time::now();
	 ...
	 while (ros::ok()){
	  if( current_state.mode != "OFFBOARD" &&
		  ...
		  if(u_c.x>1.0) u_c.x = 1.0;
		  else if(u_c.x<-1.0) u_c.x = -1.0;
		  if(u_c.y>1.0) u_c.y = 1.0;
		  else if(u_c.y<-1.0) u_c.y = -1.0;
		  ...
		  w_d_dot.x = (u_hat+Sw*Sw*d).y*(1/d.z);
		 ...
		 // printf("%f", e_int.x); 
		  ...
		  double lambda = ((u_hat +Sw*Sw*d).z+d.y*w_d_dot.x-d.x*w_d_dot.y)*m;
		 		  
		  tau = Sw*J*w + J*(w_d_dot +(w+w_d*(-1.0))*(-1.0)*k_t); // - eq/ (7)
		  ...
		  pub_y_dot.twist.linear.x = d.x;//y_dot.x;
		  pub_y_dot.twist.linear.y = d.y;//y_dot.y*(-1.0);
		  pub_y_dot.twist.linear.z = d.z;//y_dot.z*(-1.0);
